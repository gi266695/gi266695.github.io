...
class LevelInstance {
    ...
    Tile_GetTile(Vector_Point){
        ...
        return Room.Tile_GetTile(Vector_Point);
    }
}
 ...
 * @param {array} Lst_GameObjectsCanDamage 
 */
function Bool_IsBulletHitting(Lst_Levels, GameObj_Self, Lst_ObjectBoxes, Lst_GameObjectsCanDamage = null){
    if(Lst_Levels == null
            || Lst_ObjectBoxes == null
            || GameObj_Self == null
            || (Lst_GameObjectsCanDamage != null && !Array.isArray(Lst_GameObjectsCanDamage))){
    ...
    forEach_TileInBounds(Lst_Levels, Lst_ObjectBoxes, false, false, (Tile) => {
        ...
        Tile.LstObjects_GameObjects.forEach((element_GameObj) => {
            if(CheckedGameObjects == null || !CheckedGameObjects.includes(CheckedGameObjects)){
                ...
                if(Lst_GameObjectsCanDamage != null && GameObj_Self.Bool_DoesBulletDamageThis(element_GameObj)){
        ...
        if((Tile.TileDef != null && Tile.TileDef.Bool_BlocksShots && !Bool_FoundHit)
                ...
                || Lst_ObjToCheck_Hit != null
                || Lst_ObjToCheck_Damage != null){
        ...
        if(Bool_NeedToCheck){
            ...
            //we always check these becuase we have to report them
            if(Lst_ObjToCheck_Damage != null){
                for(var loopObjs = 0; loopObjs < Lst_ObjToCheck_Damage.length; loopObjs++){
                    ...
                    //report hit
                    if(Bool_HitBox){
                        Lst_GameObjectsCanDamage.push(CurrentObj);
                        if(Lst_ObjToCheck_Hit != null && Lst_ObjToCheck_Hit.includes(CurrentObj)){
                            Bool_FoundHit = true;
                        }
                    }
                    //don't check twice
                    else if(!Bool_FoundHit && Lst_ObjToCheck_Hit != null){//don't bother removing if were not going to go through them anyway
                        var index = Lst_ObjToCheck_Hit.indexOf(CurrentObj);
                        if(index >= 0 && index < Lst_ObjToCheck_Hit.length){
                            Lst_ObjToCheck_Hit.splice(index, 1);
                        }
                    }
                }
            }
        ...
        return Lst_GameObjectsCanDamage == null && Bool_FoundHit;
    });
...
function Lst_GetAgroTargets(Lst_Levels, GameObj_Self, Lst_ObjectBoxes, Func_UseObj = (Obj, Box) => {return true;}){
    ...
    forEach_TileInBounds(Lst_Levels, Lst_ObjectBoxes, false, false, (Tile) => {
        if(Tile.LstObjects_GameObjects.length > 0){
            var GameObjectsToCheck = [];
            Tile.LstObjects_GameObjects.forEach((element_GameObj) => {
            ...
            if(GameObjectsToCheck.length > 0){
                var IsColidingWithTile = false;
                for(var loop = 0; loop < Lst_ObjectBoxes.length; loop++){
                    if(Bool_GetCollisionData(Tile.HitBox, Lst_ObjectBoxes[loop])){
                        IsColidingWithTile = true;
                        break;
                    }
                }
                if(IsColidingWithTile){
...
function Vector_GetMapCollisionSolution(Lst_Levels, Lst_ObjectBoxes, Obj_Self){
    ...
    forEach_TileInBounds(Lst_Levels, Lst_ObjectBoxes, true, false, (Tile) => {
        ...
        //TODO should probubly prioritize a solution for map hhit boxes over a solution with map and game objects
        Tile.LstObjects_GameObjects.forEach((element) => {
            if(element != Obj_Self && !Lst_GameObjBoxes.includes(element)){
                Lst_GameObjBoxes.push(element);
                Lst_ObstructionBoxes.push.apply(Lst_ObstructionBoxes, element.LstBoxes_GetHitBoxesInLocalSpace(element.Str_BlocksMovement, null));
            }
        });
    });
